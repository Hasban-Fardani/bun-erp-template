...
const listRoles = async () => {
  const res = await api.app.request("/api/v1/roles", { headers: { cookie: api.cookie } });
...
async function tryDeleteRole(id: string | undefined) {
  const res = await api.app.request(`/api/v1/roles/${id}`, {
    method: "DELETE",
...
describe("roles CRUD", () => {
  ...
  test("owner creates a custom role, renames it, and assigns permissions that take effect", async () => {
    ...
    const renamed = await api.app.request(`/api/v1/roles/${role.id}`, json({ name: "Auditor Internal" }, "PATCH"));
    expect(renamed.status).toBe(200);
    expect(((await renamed.json()) as { data: { name: string } }).data.name).toBe("Auditor Internal");

    const granted = await api.app.request(
  ...
  test("replacing permissions revokes what is left out — the catalog is not additive", async () => {
    const created = await api.app.request("/api/v1/roles", json({ key: "auditor", name: "Auditor" }));
    const role = ((await created.json()) as { data: { id: string } }).data;
    await api.app.request(
      `/api/v1/roles/${role.id}/permissions`,
      json({ permissions: ["audit.read", "user.read"] }, "PUT"),
    );
    await api.app.request(`/api/v1/roles/${role.id}/permissions`, json({ permissions: ["audit.read"] }, "PUT"));
    const listed = (await listRoles()).find((r) => r.id === role.id);
    expect(listed?.permissions).toEqual(["audit.read"]);
  });
  ...
  test("custom role is deleted; system role is refused", async () => {
    const created = await api.app.request("/api/v1/roles", json({ key: "auditor", name: "Auditor" }));
    const role = ((await created.json()) as { data: { id: string } }).data;
    const removed = await api.app.request(`/api/v1/roles/${role.id}`, {
      method: "DELETE",
      headers: { cookie: api.cookie },
    });
  ...
  test("duplicate key is a conflict, not a silent overwrite", async () => {
    await api.app.request("/api/v1/roles", json({ key: "auditor", name: "Auditor" }));
    const again = await api.app.request("/api/v1/roles", json({ key: "auditor", name: "Auditor Lain" }));
  ...
  test("unknown permission is refused, and a role in use cannot be deleted", async () => {
    ...
    const bogus = await api.app.request(
      `/api/v1/roles/${role.id}/permissions`,
    ...
    const users = await api.app.request("/api/v1/users?perPage=5", { headers: { cookie: api.cookie } });
    const target = ((await users.json()) as { data: { items: { id: string }[] } }).data.items[0];
    const assigned = await api.app.request(`/api/v1/users/${target?.id}/roles`, json({ roleKey: "auditor" }));
  ...
  test("audit trail records the role changes", async () => {
    ...
    const role = ((await created.json()) as { data: { id: string } }).data;
    await api.app.request(`/api/v1/roles/${role.id}/permissions`, json({ permissions: ["audit.read"] }, "PUT"));
    await api.app.request(`/api/v1/roles/${role.id}`, { method: "DELETE", headers: { cookie: api.cookie } });

    const res = await api.app.request("/api/v1/audit-logs?perPage=50", { headers: { cookie: api.cookie } });
